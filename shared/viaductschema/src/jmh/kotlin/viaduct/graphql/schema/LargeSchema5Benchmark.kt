...
/**
 * Benchmarks for large-schema-5.
 *
 * Uses `large-schema-5.graphqls.zip` which is an anonymized version of
 * the central schema (produced by graphql-java's Anonymizer).
 *
 ...
 *
 * See also: [LargeSchemaBenchmark] for the large-schema-4 benchmarks.
 */
...
open class LargeSchema5Benchmark {
    private var schemaFound: Boolean = false
    private var benchmarkData: LargeSchemaBenchmarkData? = null

    @Setup
    fun setup() {
        val schema = LargeSchemaBenchmarkHelper.loadZippedSchema("5")
        if (schema == null) {
            println("large-schema-5 not found in classpath. Skipping benchmarks.")
            schemaFound = false
            return
        }

        schemaFound = true
        benchmarkData = LargeSchemaBenchmarkHelper.prepareBenchmarkData(schema, "large-schema-5")
    }
    ...
    @Benchmark
    fun readLargeSchema5SDLTextToGraphQLSchema(bh: Blackhole) {
        val data = benchmarkData ?: return
